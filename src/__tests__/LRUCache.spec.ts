...
describe("createLRUCache", () => {
    it("should not allow 0 as a cache size", () => {
        const expectedErrorShape = new Exception("INVALID_CACHE_SIZE", "Invalid size. The cache needs to be greater than 0 in size");
...
describe("get", () => {
    ...
    it("should return undefined if the cache key is empty", () => {
        ...
        const isInitialValueSaved = cache.put(initialValue.key, initialValue.value);
        const desiredValueToTest = {
            key: v4(),
            value: v4()
        };
        const isDesiredValueSaved = cache.put(desiredValueToTest.key, desiredValueToTest.value);
